...
class Swapper:
			...
			if run.returncode != 0:
				...
				for item in output:
					...
					if re.search(f'{pid}.*VmSwap', item):
						swap_consumed = re.sub(
							'VmSwap:\t\s+|[" "]kB', '', item).split(':')[1]
	...
	def display_data(self):
		...
		print('\033[0;30m\033[47m{: <10}{: <15}{: <10}{: <15}{: <30}\033[0m'.
			format('PID','USER', '%USAGE','USAGE in kB','COMMAND'))
		itens_to_show = 20
		for idx,proc in enumerate(data):
			...
			if idx <= itens_to_show:
				print('{: <10}{: <15}{: <10}{: <15}{: <20}'.format(pid,username,percent, swap_consumed, name))
